...
def model(settings):
    ...
    # assert math.ceil(np.sqrt(ntargets))*math.floor(np.sqrt(ntargets)) == ntargets
    # targets_x = np.linspace(0.1, 0.9, math.ceil(np.sqrt(ntargets)))
    # targets_y = np.linspace(0.1, 0.9, math.floor(np.sqrt(ntargets)))
    # targets_xx, targets_yy = np.meshgrid(targets_x, targets_y)
    ...
    misfit.noise_variance = 1.0 #noise_std_dev * noise_std_dev
