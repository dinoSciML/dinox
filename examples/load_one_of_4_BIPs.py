...
def load_one_of_4_BIPs(PDEproblem_dir: str, BIPproblem_dir: str, BIP: int) -> Tuple:
    ...
    settings["targets"] = (
        targets  # override the randomly generated targets with the one we used, saved to disk.
    )
    pde, prior, misfit, _, _ = model_module.model(settings)

    misfit.noise_variance = (
        noise_stdev * noise_stdev
    )  # set the noise variance read from file
    misfit.d.set_local(
        np.load(f"{BIPproblem_dir}BIP_data/true_Y_{BIP}.npy")
    )  # set the data read from file
...
model, true_m_np = load_one_of_4_BIPs(
    args.PDEproblem_base_dir, args.BIP_problem_dir, args.BIP
)
