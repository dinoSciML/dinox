...
for i, BIPproblem_sub_dir in enumerate(os.listdir(f"{base_folder}{PDEproblem_dir}problem")):  #(high variance may revert to the prior, low variance may revert too easily to near-gaussian around MAP ppoint)
    ...
    print(Y_data.shape)
    clustering = SpectralClustering(n_clusters=100,
                                    assign_labels='discretize',
                                    random_state=0).fit(Y_data[:,0])    
    clustering.labels_
    ...
    np.save(f"{BIPproblem_dir}BIP_data/true_sample_indices.npy",np.array(sample_indices))
    for i in range(4):
