...
def create_mean_h1_seminorm_l2_errors_and_norms(dM, batch_size):
    one_over_n_batches = 1./batch_size
    @eqx.filter_jit
    def mean_h1_seminorm_l2_errors_and_norms(nn, X, Y, dYdX, Y_L2_norms, dYdX_L2_norms, end_idx): #dM, batch_size
        predicted_Y, predicted_dYdX = value_and_jacrev(
            nn, dynamic_slice_in_dim(X, end_idx, batch_size)
        )
        mse_i, msje_i = jnp.mean(
            optax.l2_loss(
                predicted_Y.squeeze(), dynamic_slice_in_dim(Y, end_idx, batch_size)
            ),
            axis=1,
        ), jnp.mean(
            optax.l2_loss(
        ...
        return (
                ...
                one_over_n_batches * jnp.mean(
                    normalize_values(
                        mse_i, dynamic_slice_in_dim(Y_L2_norms, end_idx, batch_size)
                    )
                ), 
                one_over_n_batches * jnp.mean(
                    normalize_values(
                        msje_i, dynamic_slice_in_dim(dYdX_L2_norms, end_idx, batch_size)
                    )
                )
            )
...
def mean_l2_norm_errors_and_norms(nn, X, Y, dYdX):
    ...
    mse_i, msje_i = jnp.mean(
        ...
        optax.l2_loss(
            predicted_dYdX.squeeze(), dynamic_slice_in_dim(dYdX, end_idx, batch_size)
        )
        * dM,
        axis=(1, 2),
    )
    ...
    one_over_n_batches * jnp.mean(mse_i), one_over_n_batches * jnp.mean(
        normalize_values(mse_i, dynamic_slice_in_dim(Y_L2_norms, end_idx, batch_size))
    ), one_over_n_batches * jnp.mean(
        normalize_values(
            msje_i, dynamic_slice_in_dim(dYdX_L2_norms, end_idx, batch_size)
        )
...
def create_compute_h1_loss_metrics(dM: int, batch_size) -> Callable:
    mean_h1_seminorm_errors_and_norms = create_mean_h1_seminorm_l2_errors_and_norms(dM, batch_size)
