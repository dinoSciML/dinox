...
parser.add_argument(
    "-batch_size", "--batch_size", type=int, default=32, help="gradient batch size"
)
parser.add_argument(
    "-step_size",
    "--step_size",
...
parser.add_argument(
    "-num_epochs", "--num_epochs", type=int, default=100, help="How many epochs"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Seed for data shuffling / initialization",
)
...
parser.add_argument(
    ...
    type=str,
    default="as",
    help="choose from [as, kle / pca, None]",
)
parser.add_argument(
    "-rb_dir",
    "--rb_dir",
    ...
    help="Where to load the reduced bases from",
)
...
parser.add_argument(
    ...
    type=str,
    default="piecewise",
    help="Use LR Schedule or do not",
)
parser.add_argument(
    "-weight_decay",
    "--weight_decay",
...
train_state = train_state.TrainState.create(
    apply_fn=network.apply_fn, params=params, tx=optimizer
)
...
def compute_metrics(state, params, data, batch_size=32):
    ...
    for i_batch in range(n_batches):
        ...
        if l2_errs is None:
            ...
            l2_normalizations = jnp.concatenate(
                [l2_normalizations, l2_normalizations_i]
            )
            h1_errs = jnp.concatenate([h1_errs, h1_errs_i])
            h1_normalizations = jnp.concatenate(
                [h1_normalizations, h1_normalizations_i]
            )
    # Remainder
    if remainder > 0:
...
for epoch in range(1, args.num_epochs):
    ...
    if remainder > 0:
        batch = jax.tree_map(
            lambda x: x[shuffled_inds[end : end + remainder]], training_data
        )
        # params = optimizer.update(params,batch)
        train_state = train_step(train_state, batch)
    ...
    print(
        "Max test accuracy L2 = ",
        100 * np.max(np.array(metrics_history["test_accuracy_l2"])),
    )
    print(
        "Max test accuracy H1 (semi-norm) = ",
        100 * np.max(np.array(metrics_history["test_accuracy_h1"])),
    )
