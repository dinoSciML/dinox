...
parser.add_argument(
    "-basis_type", "--basis_type", type=str, default="as", help="pod, as or kle"
)
parser.add_argument(
    "-rank", "--rank", type=int, default=400, help="Active subspace rank"
)
# parser.add_argument(
#     # "-file_names", "--basis_type", type=str, default="mq....", help="pod, as or kle"
# )
parser.add_argument(
    "-oversample",
    "--oversample",
...
parser.add_argument(
    "-ndata", "--ndata", type=int, default=800, help="Number of samples"
)
...
if args.basis_type.lower() == "pod":
    ...
    if  all_data["q_data"].shape[0] > 1000:
    ...
    ax.set(xlabel="index", ylabel="eigenvalue", title="POD spectrum")
    ax.grid()
    ...
    if hasattr(
        ...
        m_vec = dl.Vector()
        prior.R.init_vector(m_vec, 0)  # is mvec right?
        Omega = hf.dense_to_mv_local(OmegaNumpy, m_vec)

        # if new_approach:
        Mcsr, sqrtMcsr, A_csr, mean = prior._get_parameters()
        print(
            "Creating R sparse, only do once, for all time, save to disk, maybe just load from disk"
        )
        start = time.time()
        ...
        Minv = spla.inv(Mcsr)
        print(
            "forming M_inv sparse time (will be loading from disk instead)",
            time.time() - start,
        )
        ...
        # Rinverse = Ainverse M Ainverse
        # M_inv is of the order of the same size as
        print(
            "forming R sparse time, will just load from disk isnead",
            time.time() - start,
        )
        ...
        d_GN_new, V_GN_new = hp.doublePassGAATNumpy(
            JTPhi_data, prior.R, prior, R_sparse_or_dense, OmegaNumpy, rank, s=1
        )
        # else:
    ...
    input_basis_new = hf.mv_to_dense(V_GN_new)

    # Compute the projector RV_r from the basis
    RV_GN_new = hp.MultiVector(V_GN_new[0], V_GN_new.nvec())
    RV_GN_new.zero()
    hp.MatMvMult(prior.R, V_GN_new, RV_GN_new)  # R is C^-1

    input_projector_new = hf.mv_to_dense(RV_GN_new)
    # dense_to_mv
    ...
    if check_orth:
        ...
        PsistarPsi = input_projector_new.T @ input_basis_new
        orth_error = np.linalg.norm(PsistarPsi - np.eye(PsistarPsi.shape[0]))
        print("||Psi^*Psi - I|| = ", orth_error)
        assert orth_error < 1e-5
    ...
    np.save("AS_encoder_basis_new", input_basis_new)
    np.save("AS_d_GN_new", d_GN_new)
    np.save("AS_encoder_cobasis_new", input_projector_new)

    from scipy.linalg import subspace_angles

    diff = input_projector - input_projector_new
    rel_error = np.linalg.norm(diff) / np.linalg.norm(input_projector)
    print("||RPsi.T - RPsi_new.T||/||RPsi.T|| = ", rel_error)
    print(np.rad2deg(subspace_angles(input_projector, input_projector_new)))

    # assert orth_error < 1e-5

    diff = input_basis_new - input_basis
    rel_error = np.linalg.norm(diff) / np.linalg.norm(input_basis)
    print("||Psi - Psi_new||/||Psi|| = ", rel_error)
    print(np.rad2deg(subspace_angles(input_basis_new, input_basis)))
